...
namespace FlowCanvas{
	...
	public class BinderConnection : Connection {
		...
		protected override TipConnectionStyle tipConnectionStyle{
			get {return TipConnectionStyle.None;}
		}

		protected override bool canRelink{
			get {return false;}
		}

		///Draws the binder connection
		public void DrawConnectionGUI(){
			if (sourcePort != null && targetPort != null){
				base.DrawConnectionGUI( sourcePort.pos, targetPort.pos );
			}
		}
		...
		IEnumerator Internal_BlinkStatus(Flow f){
			//yield return new WaitForSeconds(f.ticks * 0.03f); //to give the debugging a forward motion feeling
			connectionStatus = Status.Running;
			blinks ++;
			yield return null;
			blinks --;
			if (blinks == 0){
