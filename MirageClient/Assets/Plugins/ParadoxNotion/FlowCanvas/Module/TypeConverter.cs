...
namespace FlowCanvas{
	...
	public static class TypeConverter {
		...
		public static ValueHandler<object> GetConverterFuncFromTo(Type sourceType, Type targetType, ValueHandler<object> func){
			...
			if ( sourceType.GetMethods(BindingFlags.Public | BindingFlags.Static).Any( m => m.ReturnType == targetType && (m.Name == "op_Implicit" || m.Name == "op_Explicit")) )
				return func;
				...
				try
				{
					...
					if (elementTo.RTIsAssignableFrom(elementFrom)){
						return ()=>
						{
							...
							return targetType.IsArray? (object)list.ToArray() : (object)list.ToList();
						};
					}
				}
				catch {return null;}
